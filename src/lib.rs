...
impl<K, V, S: BuildHasher> Table<K, V, S> {
    ...
    fn with_capacity_and_hasher(cap: usize, hash_builder: S) -> Self {
        // The + 1 is needed to avoid losing fractional bucket to integer division.
        Table::with_hasher(cmp::max(MINIMUM_CAPACITY, cap * MAX_LOAD_FACTOR_DENOM / MAX_LOAD_FACTOR_NUM + 1), hash_builder)
    }
}
...
pub struct ReadGuard<'a, K: 'a, V: 'a, S> {
    ...
    inner: OwningRef<OwningHandle<RwLockReadGuard<'a, Table<K, V, S>>, RwLockReadGuard<'a, Bucket<K, V>>>, V>,
}
...
pub struct WriteGuard<'a, K: 'a, V: 'a, S> {
    /// The inner hecking long type.
    inner: OwningHandle<OwningHandle<RwLockReadGuard<'a, Table<K, V, S>>, RwLockWriteGuard<'a, Bucket<K, V>>>, &'a mut V>,
}
...
where S: Default + BuildHasher
 {
    ...
    pub fn clear(&self) -> CHashMap<K, V, S> {
        ...
        CHashMap {
            ...
            table: RwLock::new(mem::replace(&mut *lock, Table::with_hasher(DEFAULT_INITIAL_CAPACITY, S::default()))),
...
where K: PartialEq + Hash,
    ...
    pub fn reserve(&self, additional: usize) {
        ...
        if lock.buckets.len() < len {
            ...
            let table = mem::replace(&mut *lock, Table::with_capacity_and_hasher(len, S::default()));
    ...
    pub fn shrink_to_fit(&self) {
        ...
        let mut lock = self.table.write();
        // Swap the table out with a new table of desired size (multiplied by some factor).
        let table = mem::replace(&mut *lock, Table::with_capacity_and_hasher(self.len(), S::default()));
        // Fill the new table with the data from the old table.
        lock.fill(table);
    }
...
impl<K: PartialEq + Hash, V, S: Default + BuildHasher> iter::FromIterator<(K, V)> for CHashMap<K, V, S> {
